...
const
  CBancoCecred_Versao = '0.0.1';

type
  { TBancoCecred}

  TBancoCecred = class(TBancoClass)
   protected
    FCountRegR:Integer;
   private
...
constructor TBancoCecred.create(AOwner: TBanco);
begin
...
function TBancoCecred.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
   Result := '0';
...
function TBancoCecred.CalcularTamMaximoNossoNumero(
  const Carteira: String; NossoNumero : String = ''; Convenio: String = ''): Integer;
begin
   Result := 17;

   if (Banco.Boleto.Cedente.Conta = '') then
      raise Exception.Create(Str('Banco Cecred requer que a Conta '+
                                     'seja informada.'));

end;

function TBancoCecred.FormataNossoNumero(const Titulo :TTitulo): String;
var
  ANossoNumero : String;
  AConta : String;
begin
   ...
   begin
      ...
      AConta    := IntToStr(StrToInt64(OnlyNumber(Boleto.Cedente.Conta + Boleto.Cedente.ContaDigito)));
...
function TBancoCecred.MontarCodigoBarras(const Titulo : TTitulo): String;
var
  CodigoBarras: String;
  ...
  AConvenio: String;
begin
   AConvenio    := IntToStr(StrToInt64Def(OnlyNumber(Titulo.Boleto.Cedente.Convenio),0));
   ANossoNumero := IntToStrZero(StrToIntDef(OnlyNumber(Titulo.NossoNumero),0),9);
...
function TBancoCecred.MontarCampoCodigoCedente (
...
function TBancoCecred.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
var
  ANossoNumero: String;
begin
...
function TBancoCecred.GerarRegistroHeader240(
...
begin
  ...
  begin
    ...
    Result:= IntToStrZero(Banco.Numero, 3)               + // 1 a 3 - Código da cooperativa
    ...
    Result:= Result + #13#10 +
             IntToStrZero(Banco.Numero, 3)               + // 1 a 3 - Código da cooperativa
...
procedure TBancoCecred.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa:TStringList);
var
  aAgencia: String;
  aConta: String;
  aConvenio : String;
...
function TBancoCecred.GerarRegistroTransacao240(Titulo: TTitulo): String;
var
...
procedure TBancoCecred.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
  ANossoNumero: String;
...
begin
   ...
   begin
      ANossoNumero := FormataNossoNumero(Titulo);
      aAgencia:= IntToStrZero(StrToIntDef(OnlyNumber(Boleto.Cedente.Agencia),0),4);
      aConta  := IntToStrZero(StrToIntDef(OnlyNumber(Boleto.Cedente.Conta),0),8);
      aModalidade := IntToStrZero(StrToIntDef(trim(Boleto.Cedente.Modalidade),0),3);
...
function TBancoCecred.GerarRegistroTrailler240(ARemessa: TStringList): String;
var
  wQTDTitulos : Integer;
begin
...
procedure TBancoCecred.GerarRegistroTrailler400(
...
function TBancoCecred.TipoOCorrenciaToCod (
...
function TBancoCecred.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoCecred.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoCecred.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoCecred.CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia; CodMotivo: Integer): String;
begin
  case TipoOcorrencia of
...
procedure TBancoCecred.LerRetorno240(ARetorno: TStringList);
var
  Titulo: TTitulo;
...
procedure TBancoCecred.LerRetorno400(ARetorno: TStringList);
var
  Titulo : TTitulo;
...
begin
   ...
   Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0],41,6),0);
