unit Myloo.Boleto.BancoSicredi;
...
type
  ...
  TBancoSicredi = class(TBancoClass)
  private
    FBoleto: TBoleto;
  ...
  public
    ...
    Function TipoDescontoToString(const AValue: TTipoDesconto): String; override;
    ...
    Function CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia;CodMotivo:String): String; override;
...
constructor TBancoSicredi.create(AOwner: TBanco);
begin
...
function TBancoSicredi.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
   Modulo.CalculoPadrao;
   Modulo.Documento := Titulo.Boleto.Cedente.Agencia +
                       PadLeft(Titulo.Boleto.Cedente.AgenciaDigito, 2, '0') +
                       PadLeft(Titulo.Boleto.Cedente.CodigoCedente, 5, '0') +
                       FormatDateTime('yy',Titulo.DataDocumento) +
                       Titulo.CodigoGeracao + RightStr(Titulo.NossoNumero,5);
   Modulo.Calcular;
...
function TBancoSicredi.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
...
begin
   ...
   begin
      ...
      CampoLivre :=   Modalidade                                      + { 1-Com registro ou 3-Sem registro. Por enquanto vou deixar 1 mais tenho que tratar menhor essa informação }
                      ...
                      OnlyNumber(MontarCampoNossoNumero(Titulo))  +
      ...
      CodigoBarras := IntToStr( Numero )                                     + { Código do banco 748 }
                      ...
                      IntToStrZero(Round(Titulo.ValorDocumento*100),10)  + { valor do documento }
...
function TBancoSicredi.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
begin
  Result:= FormatDateTime('yy',Titulo.DataDocumento) + '/' +
           Titulo.CodigoGeracao + RightStr(Titulo.NossoNumero,5) + '-' +
           CalcularDigitoVerificador(Titulo);
end;

function TBancoSicredi.MontarCampoCodigoCedente (const Titulo: TTitulo ) : String;
begin
...
procedure TBancoSicredi.GerarRegistroHeader400(NumeroRemessa : Integer; aRemessa: TStringList);
var wLinha: String;
begin
...
procedure TBancoSicredi.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   with Titulo do
   begin
      wNossoNumeroCompleto := OnlyNumber(MontarCampoNossoNumero(Titulo));
      ...
      if (Boleto.Cedente.ResponEmissao <> tbCliEmite) or 
      ...
      if StrToIntDef(Boleto.Cedente.Modalidade,1) = 1 then
         wModalidade := 'A'
      else
         wModalidade := 'C'; 
      ...
      begin
         ...
         begin
           ...
           wLinha:=  '5'                                                         + // 001 a 001 - Identificação do registro Informativo
                     ...
                     PadLeft( Banco.Boleto.Cedente.CodigoCedente, 5, '0')+ // 003 a 004 - Codigo do Cedente
...
procedure TBancoSicredi.GerarRegistroTrailler400( ARemessa:TStringList );
var
  wLinha: String;
begin
  with Banco.Boleto.Cedente do begin
...
procedure TBancoSicredi.LerRetorno400(ARetorno: TStringList);
var
  Titulo : TTitulo;
...
begin
  ...
  Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0],111,7),0);

  Banco.Boleto.DataArquivo := StringToDateTimeDef(Copy(ARetorno[0],101,2)+'/'+
  ...
  begin
    if (not LeCedenteRetorno) and (rCodCedente <> OnlyNumber(Cedente.CodigoCedente)) then
       raise Exception.Create(Str('Agencia\Conta do arquivo inválido'));
...
function TBancoSicredi.CodMotivoRejeicaoToDescricao(
...
function TBancoSicredi.CalcularNomeArquivoRemessa: String;
var
         ...
         begin
           ...
           begin
             if Sequencia = 10 then
               ...
               raise Exception.Create(Str('Número máximo de 10 arquivos '+
...
function TBancoSicredi.TipoDescontoToString(const AValue: TTipoDesconto): string;
begin
  ...
  if self.Banco.Boleto.LayoutRemessa = c400 then
    begin
      case AValue of
...
function TBancoSicredi.TipoOcorrenciaToDescricao(
...
function TBancoSicredi.CodOcorrenciaToTipo(
...
function TBancoSicredi.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoSicredi.TipoOCorrenciaToCod(
  const TipoOcorrencia: TTipoOcorrencia): String;
begin
  Result := '';
...
function TBancoSicredi.GerarRegistroHeader240(
  NumeroRemessa: Integer): String;
var TipoInsc: String;
begin
  case Banco.Boleto.Cedente.TipoInscricao of
    pFisica:   TipoInsc := '1';
...
function TBancoSicredi.GerarRegistroTrailler240(
  ARemessa: TStringList): String;
begin
   {REGISTRO TRAILER DO LOTE}
   Result:= IntToStrZero(Banco.Numero, 3)           + // 001 a 003 - Código do banco na compensação
...
function TBancoSicredi.GerarRegistroTransacao240(
...
var
    ...
    TipoAvalista: Char;
begin
  with Banco.Boleto.Cedente, Titulo do
  begin
    {Aceite}
    case Aceite of
      atSim: AceiteStr := 'A';
    ...
    Result:= '748'                                                            + // 001 a 003 - Código do banco na compensação
             ...
             IntToStrZero(
               (3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 1 , 5)   + // 009 a 013 - Nº sequencial do registro do lote
             ...
             PadRight(OnlyNumber(MontarCampoNossoNumero(Titulo)), 20, '0')+ // 038 a 057 - Identificação do título no banco
             ...
             TipoDescontoToString(Titulo.TipoDesconto)                    + // 142 a 142 - Código do desconto 1
    ...
    Result:= Result + #13#10 +
             ...
             '3'                                                            + // 008 a 008 - Tipo de registro = "3" DETALHE
             IntToStrZero(
               (3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 2 , 5) + // 009 a 013 - Nº sequencial do registro do lote
      ...
      Result:= Result + #13#10 +
               ...
               IntToStrZero(
               (3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 3 , 5)+ // 9 a 13 - Número seqüencial do registro no lote - Cada registro possui dois segmentos
               'R'                                                         + // Código do segmento do registro detalhe
               ' '                                                         + // Uso exclusivo FEBRABAN/CNAB: Branco
               '01'                                                        + // 16 a 17 - Código de movimento
               TipoDescontoToString(Titulo.TipoDesconto2)              + // 18  tipo de desconto 2
               ...
               TipoDescontoToString(Titulo.TipoDesconto2)              + // 42 tipo de desconto 3
...
procedure TBancoSicredi.LerRetorno240(ARetorno: TStringList);
var Titulo: TTitulo;
    SegT, SegU: String;
