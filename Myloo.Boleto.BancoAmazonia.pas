unit Myloo.Boleto.BancoAmazonia;
...
const
  CBancoAmazonia_Versao = '0.0.1';

type
  { TBancoAmazonia }

  TBancoAmazonia = class(TBancoClass)
  protected
  private
    FBoleto: TBoleto;
...
constructor TBancoAmazonia.Create(AOwner: TBanco);
begin
...
function TBancoAmazonia.CalcularNomeArquivoRemessa: String;
var
  Sequencia   :Integer;
...
function TBancoAmazonia.CalcularDigitoVerificador(const Titulo: TTitulo): String;
var
...
function TBancoAmazonia.CalcularTamMaximoNossoNumero(const Carteira: String; NossoNumero: String = ''; Convenio: String = ''): Integer;
var
...
begin
  ...
  if (Banco.Boleto.Cedente.Convenio = '') then
    raise Exception.create(Str('Banco da Amazonia requer que o número do convênio seja informado.'));
  wTamConvenio := Length(Trim(Banco.Boleto.Cedente.Convenio));
  if (wTamConvenio <= 4) and (Result <> 20) then
    Result := 16;
end;

function TBancoAmazonia.FormataNossoNumero(const Titulo: TTitulo): String;
var
  ANossoNumero: String;
...
function TBancoAmazonia.MontarCodigoBarras(const Titulo: TTitulo): String;
var
  ...
  begin
    ...
    if Titulo.Carteira = '001' Then
      CodigoBarras := PadLeft(OnlyNumber(IntToStr(fpNumero)), 3, '0') +
                      ...
                      RightStr(OnlyNumber(Titulo.NossoNumero),7) +
                      FormatDateTime('DDMMYY',Titulo.Vencimento) +
                      '00000000'
    else
      CodigoBarras := PadLeft(OnlyNumber(IntToStr(fpNumero)), 3, '0') +
                      '9' +
                      ...
                      PadLeft(OnlyNumber(Titulo.NossoNumero), FpTamanhoMaximoNossoNum, '0') +
                      '8';
...
function TBancoAmazonia.MontarCampoCodigoCedente(const Titulo: TTitulo): String;
begin
...
function TBancoAmazonia.MontarCampoNossoNumero(const Titulo: TTitulo): String;
var
  ANossoNumero: string;
  wTamConvenio: Integer;
begin
  ...
  if (wTamConvenio = 4) and (Titulo.Carteira <> '001')  then
    Result := ANossoNumero + '.' + CalcularDigitoVerificador(Titulo)
  else
    Result := ANossoNumero;
end;

function TBancoAmazonia.GerarRegistroHeader240(NumeroRemessa: Integer): String;
var
...
begin
  ...
  begin
    ...
    Result := Result + #13#10 + IntToStrZero(Banco.Numero, 3) +          // 1 a 3 - Código do banco
...
function TBancoAmazonia.GerarRegistroTransacao240(Titulo: TTitulo): String;
var
...
begin
  ...
  begin
    ANossoNumero := FormataNossoNumero(Titulo);
    wTamConvenio := Length(Banco.Boleto.Cedente.Convenio);
    wTamNossoNum := CalcularTamMaximoNossoNumero(Titulo.Carteira, Titulo.NossoNumero);
    wCarteira := StrToIntDef(Carteira, 0);
    if (Integer(Boleto.Cedente.TipoDocumento) > 0) then
      wTipoDocumento := Integer(Boleto.Cedente.TipoDocumento)
    else wTipoDocumento := 1;
    if (((wCarteira = 11) or (wCarteira = 31) or (wCarteira = 51)) or
       ((wCarteira = 12) or (wCarteira = 15) or (wCarteira = 17))) and
       ((Boleto.Cedente.ResponEmissao <> tbCliEmite) and (StrToIntDef(NossoNumero, 0) = 0)) then
    begin
    ...
    Result := IntToStrZero(Banco.Numero, 3) +                                                                      // 1 a 3 - Código do banco
         ...
         IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 1, 5) +                                    // 9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
         ...
         PadRight(Boleto.Cedente.AgenciaDigito, 1, '0') +                                                          // 106 Digito da agencia cobradora
    ...
    Result := Result + #13#10 + IntToStrZero(Banco.Numero, 3) +                                                    // 1 a 3 - Código do banco
         ...
         '3' +                                                                                                         // 8 - Tipo do registro: Registro detalhe
         IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 2, 5) +                                    // 9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
    ...
    { SEGMENTO R }
    Result := Result + #13#10 + IntToStrZero(Banco.Numero, 3) +                                                    // 1 - 3 Código do banco
         '0001' +                                                                                                      // 4 - 7 Número do lote
         '3' +                                                                                                         // 8 - 8 Tipo do registro: Registro detalhe
         IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 3, 5) +                                    // 9 - 13 Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
...
function TBancoAmazonia.GerarRegistroTrailler240(ARemessa: TStringList): String;
var
  aFEBRABAN : string;
begin
...
procedure TBancoAmazonia.GerarRegistroHeader400(NumeroRemessa: Integer; ARemessa: TStringList);
begin
     raise Exception.create(Str('CNAB 400 não implementado.'));
end;

procedure TBancoAmazonia.GerarRegistroTransacao400(Titulo: TTitulo; ARemessa: TStringList);
begin
     raise Exception.create(Str('CNAB 400 não implementado.'));
end;

procedure TBancoAmazonia.GerarRegistroTrailler400(ARemessa: TStringList);
begin
     raise Exception.create(Str('CNAB 400 não implementado.'));
end;

procedure TBancoAmazonia.LerRetorno240(ARetorno: TStringList);
var
     Titulo: TTitulo;
...
begin
     ...
     if StrToIntDef(Copy(ARetorno.Strings[0], 1, 3), -1) <> Numero then
          raise Exception.create(Str(Banco.Boleto.NomeArqRetorno + 'não' + 'é um arquivo de retorno do ' + Nome));
          ...
          if Assigned(Titulo) then
               ...
               begin
                    ...
                    begin
                         ...
                         if Length(Boleto.Cedente.Convenio) = 6 then
                           NossoNumero := Copy(Linha, 44, Banco.TamanhoMaximoNossoNum)
                         else
                           NossoNumero := Copy(Linha, 38, Banco.TamanhoMaximoNossoNum);
...
function TBancoAmazonia.TipoOCorrenciaToCod(const TipoOcorrencia: TTipoOcorrencia): String;
begin
     Result := '';
...
function TBancoAmazonia.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoAmazonia.CodOcorrenciaToTipo(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
...
function TBancoAmazonia.CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia; CodMotivo: Integer): String;
begin
     case TipoOcorrencia of
...
procedure TBancoAmazonia.LerRetorno400(ARetorno: TStringList);
var
...
procedure TBancoAmazonia.LerRetorno400Pos6(ARetorno: TStringList);
var
     Titulo: TTitulo;
...
procedure TBancoAmazonia.LerRetorno400Pos7(ARetorno: TStringList);
var
     Titulo: TTitulo;
