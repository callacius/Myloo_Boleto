unit Myloo.Boleto.BancoCaixaSICOB;
...
type
  ...
  { TCaixaEconomicaSICOB}

  TCaixaEconomicaSICOB = class(TBancoClass)
   protected
   ...
   public
    ...
    Function CalcularDVCedente(const Titulo: TTitulo; UsaAgencia: boolean = False ): String;
...
uses
  StrUtils, Variants, math, DateUtils, Myloo.Boleto.Utils;

constructor TCaixaEconomicaSICOB.create(AOwner: TBanco);
begin
   ...
   FpOrientacoesBanco.Add(Str('SAC CAIXA: 0800 726 0101 (informações, reclamações, sugestões e elogios) ' + sLineBreak+
                          'Para pessoas com deficiência auditiva ou de Fala: 0800 726 2492 ' + sLineBreak +
                          'Ouvidoria: 0800 725 7474') + sLineBreak+
                          '     caixa.gov.br      ');
end;

function TCaixaEconomicaSICOB.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
var
  Num, Res :String;
begin
   Result := '0';
   Num := OnlyNumber(FormataNossoNumero(Titulo));
...
function TCaixaEconomicaSICOB.CalcularDVAgCD(Header: Boolean): string;
var
...
function TCaixaEconomicaSICOB.CalcularDVCedente(const Titulo: TTitulo;
   UsaAgencia: boolean = False): String;
var
  Num, Res: string;
begin
  if UsaAgencia then
     Num:=  RightStr(Titulo.Boleto.Cedente.Agencia,4) +
            Copy(Titulo.Boleto.Cedente.CodigoCedente,
                 Length(Titulo.Boleto.Cedente.CodigoCedente)-10,11)
  else
     Num := copy(Titulo.Boleto.Cedente.CodigoCedente,
                 Length(Titulo.Boleto.Cedente.CodigoCedente)-7,8);
...
function TCaixaEconomicaSICOB.CodMotivoRejeicaoToDescricao(
...
function TCaixaEconomicaSICOB.CalcularTamMaximoNossoNumero(
...
function TCaixaEconomicaSICOB.CodOcorrenciaToTipo(
...
function TCaixaEconomicaSICOB.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TCaixaEconomicaSICOB.GetLocalPagamento: String;
begin
  Result := Str(CInstrucaoPagamentoLoterica);
end;

function TCaixaEconomicaSICOB.FormataNossoNumero(const Titulo :TTitulo): String;
var
  ANossoNumero: String;
  wOperacao: Integer;
begin
   ...
   begin
     ...
     wOperacao    := StrToIntDef(Copy(Boleto.Cedente.Convenio, 1 , 3 ), 0);

     if (Carteira = 'SR') then
      begin
       if (wOperacao =  870) and (Boleto.Cedente.TipoCarteira = tctEletronica) then
...
function TCaixaEconomicaSICOB.MontarCodigoBarras(const Titulo : TTitulo): String;
var
...
begin
   ...
   aCodCedente:= PadLeft(RightStr(Titulo.Boleto.Cedente.CodigoCedente,11),11,'0');

   {Montando Campo Livre}
   if Length(ANossoNumero) > 11 then
   begin                                                                  //                     Tamanho
     CampoLivre := RightStr(aCodCedente, 5) +                             // Código do Cedende - 05
                   RightStr(Titulo.Boleto.Cedente.Agencia, 4) +   // Agência           - 04
                   '87' +                                                 // Constante         - 02
   ...
   begin
     CampoLivre := ANossoNumero +
                   RightStr(Titulo.Boleto.Cedente.Agencia, 4) +
                   aCodCedente;
   end;
...
function TCaixaEconomicaSICOB.RetornaCodCarteira(Carteira: string): integer;
begin
if Carteira = 'CS' then
  Result := 11
else if Carteira = 'CR' then
...
function TCaixaEconomicaSICOB.TipoOCorrenciaToCod(
...
function TCaixaEconomicaSICOB.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TCaixaEconomicaSICOB.MontarCampoCodigoCedente (
...
begin
  ...
  begin
     Result := RightStr(Cedente.Agencia,4) + '.'+
               ...
               '-' +CalcularDVCedente(Titulo,true);

  end;
end;

function TCaixaEconomicaSICOB.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
...
function TCaixaEconomicaSICOB.GerarRegistroHeader240(NumeroRemessa : Integer): String;
var
...
begin
   ...
   begin
      ...
      ACodCedenteDVAg := CalcularDVCedente(Banco.Boleto.ListadeBoletos[0],True);
      ACodCedenteDV   := CalcularDVCedente(Banco.Boleto.ListadeBoletos[0]);
...
procedure TCaixaEconomicaSICOB.GerarRegistroHeader400(
...
begin
   ...
   begin
     ...
     ACodCedente := PadLeft(OnlyNumber(MontarCampoCodigoCedente(Banco.Boleto.ListadeBoletos[0])),16,'0');
...
function TCaixaEconomicaSICOB.GerarRegistroTransacao240(Titulo : TTitulo): String;
var
...
begin
   ...
   with Titulo do
   begin
      ANossoNumero := FormataNossoNumero(Titulo)+CalcularDigitoVerificador(Titulo);
      ...
      aCodCedente:= RightStr(Boleto.Cedente.CodigoCedente,8);

      Result:= IntToStrZero(Banco.Numero, 3)                                        + //   1 a   3 - Código do banco
               '0001'                                                                   + //   4 a   7 - Lote de Serviço
               '3'                                                                      + //   8 a   8 - Tipo do registro: Registro detalhe
               IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo))+ 1 ,5)  + //   9 a  13 - Nº Sequencial do Registro no Lote
               ...
               PadLeft(Boleto.Cedente.AgenciaDigito, 1 , '0')                       + //  23 a  23 - Dígito verificador da agência
      ...
      Result:= Result + #13#10 +
               ...
               '0001'                                                                  + // 4 a 7 - Lote de Serviço
               '3'                                                                     + //   8 a   8 - Tipo do registro: Registro detalhe
               IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo))+ 2 ,5) + //   9 a  13 - Número do lote
      ...
      Result:= Result + #13#10 +
               ...
               '3'                                                                     + //   8 a 8   - Tipo do registro: Registro detalhe
               IntToStrZero((3 * Boleto.ListadeBoletos.IndexOf(Titulo))+ 3 ,5) + //   9 a 13  - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
...
procedure TCaixaEconomicaSICOB.GerarRegistroTransacao400(
  Titulo: TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   begin
     ...
     ACodCedente := PadLeft(OnlyNumber(MontarCampoCodigoCedente(Titulo)),16,'0');
...
function TCaixaEconomicaSICOB.GerarRegistroTrailler240( ARemessa : TStringList ): String;
begin
...
procedure TCaixaEconomicaSICOB.GerarRegistroTrailler400(
...
procedure TCaixaEconomicaSICOB.LerRetorno240(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
...
begin
   ...
   case StrToIntDef(Copy(ARetorno[1],18,1),0) of
     1: Banco.Boleto.Cedente.TipoInscricao:= pFisica;
     else
        Banco.Boleto.Cedente.TipoInscricao:= pJuridica;
   end;
   ...
   begin
      ...
      begin
         if Copy(Linha,14,1)= 'T' then //segmento T
          begin
            Banco.TamanhoMaximoNossoNum :=
             CalcularTamMaximoNossoNumero(Carteira, '', Banco.Boleto.Cedente.Convenio);
...
procedure TCaixaEconomicaSICOB.LerRetorno400(ARetorno: TStringList);
var
  Titulo : TTitulo;
