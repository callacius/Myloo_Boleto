unit Myloo.Boleto.BancoSafraBradesco;
...
type
  TBancoSafraBradesco = class(TBancoClass)
  private
    FBoleto: TBoleto;
...
constructor TBancoSafraBradesco.Create(AOwner: TBanco);
begin
...
function TBancoSafraBradesco.CalcularDigitoVerificador(const Titulo: TTitulo): String;
begin
  Modulo.CalculoPadrao;
  Modulo.MultiplicadorFinal := 7;
  Modulo.Documento := Titulo.Carteira + FormatDateTime('YY', Date) + Titulo.NossoNumero;
  Modulo.Calcular;
...
function TBancoSafraBradesco.MontarCodigoBarras(const Titulo: TTitulo): String;
var
...
function TBancoSafraBradesco.MontarCampoNossoNumero(const Titulo: TTitulo): String;
begin
  Result:= Titulo.Carteira + '/' + FormatDateTime('YY', Date) + ' ' +
           Titulo.NossoNumero + '-' + CalcularDigitoVerificador(Titulo);
end;

function TBancoSafraBradesco.MontarCampoCodigoCedente(const Titulo: TTitulo): String;
begin
  Result :=
    Titulo.Boleto.Cedente.Agencia + '-' +
    Titulo.Boleto.Cedente.AgenciaDigito + '/' +
    Titulo.Boleto.Cedente.Conta + '-' +
    Titulo.Boleto.Cedente.ContaDigito;
end;

procedure TBancoSafraBradesco.GerarRegistroHeader400(NumeroRemessa: Integer; ARemessa: TStringList);
var
  wLinha: String;
begin
  aTotal := 0;
...
procedure TBancoSafraBradesco.GerarRegistroTransacao400(Titulo: TTitulo; ARemessa: TStringList);
var
...
procedure TBancoSafraBradesco.GerarRegistroTrailler400(ARemessa: TStringList);
var
  wLinha: String;
begin
  wLinha :=
...
procedure TBancoSafraBradesco.LerRetorno400(ARetorno: TStringList);
var
  Titulo: TTitulo;
  ContLinha: Integer;
  CodMotivo: Integer;
...
begin
  ...
  rAgencia     := Trim(Copy(ARetorno[0], 27, Banco.TamanhoAgencia));
  rConta       := Trim(Copy(ARetorno[0], 32, Banco.TamanhoConta));
  rDigitoConta := Trim(Copy(ARetorno[0], 40, 1));

  Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0], 392, 3), 0);

  Banco.Boleto.DataArquivo :=
    StringToDateTimeDef(
...
function TBancoSafraBradesco.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoSafraBradesco.CodOcorrenciaToTipo(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoSafraBradesco.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoSafraBradesco.TipoOcorrenciaToCod(const TipoOcorrencia: TTipoOcorrencia): String;
begin
...
function TBancoSafraBradesco.CodMotivoRejeicaoToDescricao(
