unit Myloo.Boleto.BancoNordeste;
...
type
  ...
  TBancoNordeste = class(TBancoClass)
  private
    FSequencia: Integer;
    FBoleto: TBoleto;
...
constructor TBancoNordeste.create(AOwner: TBanco);
begin
...
function TBancoNordeste.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
...
function TBancoNordeste.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
...
begin
   ...
   begin
      ...
      CodigoBarras := IntToStrZero( Numero, 3 )+'9'+ FatorVencimento +
                      ...
                      CarteiraToTipoOperacao(Titulo.Carteira) + '000';
...
function TBancoNordeste.MontarCampoNossoNumero (
   const Titulo: TTitulo ) : String;
begin
   Result:= Titulo.NossoNumero+'-'+CalcularDigitoVerificador(Titulo);
end;

function TBancoNordeste.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
...
function TBancoNordeste.MontarCampoCarteira(const Titulo: TTitulo
  ): String;
begin
  Result:= CarteiraToTipoOperacao(Titulo.Carteira);
end;

procedure TBancoNordeste.GerarRegistroHeader400(NumeroRemessa : Integer; aRemessa:TStringList);
var
  wLinha: String;
begin
...
procedure TBancoNordeste.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   begin
      ...
      aConta   := IntToStrZero(StrToIntDef(OnlyNumber(Boleto.Cedente.Conta),0),7) + IntToStrZero(StrToIntDef(trim(Boleto.Cedente.ContaDigito),0),1);
      ...
      if Boleto.Cedente.CaracTitulo = tcSimples then
      begin
      ...
      else if Boleto.Cedente.CaracTitulo = tcVinculada then
      begin
        if Boleto.Cedente.ResponEmissao = tbBancoEmite then
          wCarteira:= '2'
        else
...
procedure TBancoNordeste.GerarRegistroTrailler400( ARemessa:TStringList );
var
  wLinha: String;
begin
...
procedure TBancoNordeste.LerRetorno400(ARetorno: TStringList);
var
  Titulo : TTitulo;
  ContLinha, i: integer;
...
begin
   ...
   rAgencia := trim(Copy(ARetorno[0], 27, Banco.TamanhoAgencia));   // alterado de linha 1 para 0 e posição 26 p/ 27 p/ LP Sistemas em 01/12/2015
   rConta   := trim(Copy(ARetorno[0], 33, Banco.TamanhoConta));       // alterado de linha 1 para 0 e posição 31 p/ 33 p/ LP Sistemas em 01/12/2015
...
function TBancoNordeste.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoNordeste.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoNordeste.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoNordeste.TipoOCorrenciaToCod (
...
function TBancoNordeste.CarteiraToTipoOperacao(const Carteira: string):String; 
begin
  if Carteira = '01' then
    Result:= '21'
...
function TBancoNordeste.CodMotivoRejeicaoToDescricao(
...
function TBancoNordeste.MotivoRejeicaoColuna(const Coluna: integer):string; //override;
begin
  case Coluna of
