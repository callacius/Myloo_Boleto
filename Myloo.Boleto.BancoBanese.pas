...
type
  ...
  TBancoBanese = class(TBancoClass)
  private
  ...
  public
    ...
    Function CalcularCampoASBACE(const Titulo: TTitulo):string;
...
uses
  StrUtils, Variants, FileCtrl, Myloo.Boleto.Utils;



{ TBancoBanese }

constructor TBancoBanese.create(AOwner: TBanco);
begin
...
function TBancoBanese.GetASBACE: string;
begin
...
function TBancoBanese.CalcularCampoASBACE(
...
begin
  ...
  ANossoNumero    := Copy(Trim(Titulo.NossoNumero), 1, Titulo.Boleto.Banco.TamanhoMaximoNossoNum);
  AContaComDigito := IntToStrZero(StrToIntDef(Trim(Titulo.Boleto.Cedente.Conta),0),8)+Titulo.Boleto.Cedente.ContaDigito;
  AContaComDigito := Copy(Trim(AContaComDigito), 1, Titulo.Boleto.Banco.TamanhoConta);

  cLivreAsbace := Trim(Titulo.Boleto.Cedente.Agencia)+
...
function TBancoBanese.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
var
...
function TBancoBanese.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
  CodigoBarras,FatorVencimento, DigitoCodBarras:String;
begin
  with Titulo.Boleto do
  begin
    FASBACE := CalcularCampoASBACE(Titulo);
    ...
    CodigoBarras := CodigoBarras + IntToStrZero(Round(Titulo.ValorDocumento*100),10);
...
function TBancoBanese.MontarCampoNossoNumero (
   const Titulo: TTitulo ) : String;
begin
   Titulo.NossoNumero     := IntToStrZero( StrToIntDef((Trim(Titulo.NossoNumero)+Trim(CalcularDigitoVerificador(Titulo))),0) ,Self.TamanhoMaximoNossoNum);
   Result := Titulo.NossoNumero;
end;

function TBancoBanese.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
   Result := Titulo.Boleto.Cedente.Agencia+ '/' +
             IntToStrZero(StrToIntDef(Trim(Titulo.Boleto.Cedente.Conta),0),8)+Titulo.Boleto.Cedente.ContaDigito;
end;


function TBancoBanese.FormataNossoNumero(const Titulo :TTitulo): String;
var
...
function TBancoBanese.RetornaModalidade(const Titulo :TTitulo): String;
var
  AModalidade : String;
begin
...
function TBancoBanese.CodigoLiquidacao_Descricao(CodLiquidacao: Integer): String;
begin
...
procedure TBancoBanese.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa: TStringList );
var
  wLinha: String;
begin
...
procedure TBancoBanese.GerarRegistroTrailler400(ARemessa: TStringList);
var
  wLinha: String;
begin
...
procedure TBancoBanese.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   begin
      ...
      begin
        ...
        wLinha := wLinha + IntToStrZero(aRemessa.Count + 1 {ListadeBoletos.IndexOf(Titulo) +
                               ListadeBoletos.IndexOf(Titulo) + 2}, 6);
        aRemessa.Text := aRemessa.Text + UpperCase(wLinha);
      end;
   end;
...
function TBancoBanese.GerarRegistroHeader240(NumeroRemessa : Integer): String;
var
...
function TBancoBanese.GerarRegistroTransacao240(Titulo : TTitulo): String;
var
...
begin
  ...
  begin
      ...
      Result:= IntToStrZero(Banco.Numero, 3)                          + //1 a 3 - Código do banco
               ...
               PadLeft(Boleto.Cedente.AgenciaDigito, 1 , '0')         + //23 -Dígito verificador da agência
               PadLeft(Boleto.Cedente.Conta, 12, '0')                 + //24 a 35 - Número da Conta Corrente
               PadLeft(Boleto.Cedente.ContaDigito, 1, '0')            + //36 a 36 - Dígito Verificador da Conta
...
function TBancoBanese.GerarRegistroTrailler240( ARemessa : TStringList ): String;
var
  wQTDTitulos: Integer;
begin
...
procedure TBancoBanese.LerRetorno240(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
...
procedure TBancoBanese.LerRetorno400(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
...
function TBancoBanese.TipoOcorrenciaToDescricao(
...
function TBancoBanese.CodOcorrenciaToTipo(
...
function TBancoBanese.TipoOCorrenciaToCod(
  const TipoOcorrencia: TTipoOcorrencia): String;
begin
  Result := '';
...
function TBancoBanese.CodMotivoRejeicaoToDescricao(
