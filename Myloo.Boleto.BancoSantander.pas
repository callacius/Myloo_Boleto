unit Myloo.Boleto.BancoSantander;
...
type
  ...
  TBancoSantander = class(TBancoClass)
  private
    FBoleto: TBoleto;
...
uses
  dateutils, StrUtils, math, Myloo.Boleto.Utils;

{ TBancoSantander }

constructor TBancoSantander.create(AOwner: TBanco);
begin
...
function TBancoSantander.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
...
function TBancoSantander.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
...
begin
   ...
   begin
      ...
      CodigoBarras := '033'+'9'+ FatorVencimento +
                       ...
                       PadLeft(Titulo.NossoNumero + DigitoNossoNumero, 13,'0') +
...
function TBancoSantander.MontarCampoNossoNumero (
...
begin
   ...
   Result:= PadLeft(Titulo.NossoNumero,12,'0')+ ' '+ CalcularDigitoVerificador(Titulo);
end;

function TBancoSantander.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
...
function TBancoSantander.GerarRegistroHeader240(NumeroRemessa: Integer): String;
begin
...
procedure TBancoSantander.GerarRegistroHeader400(NumeroRemessa : Integer; aRemessa: TStringList);
var
  wLinha: String;
begin
   vTotalTitulos:= 0;
...
function TBancoSantander.GerarRegistroTransacao240(Titulo: TTitulo): String;
var
  ISequencia: Integer;
...
begin
 aTipoInscricao := ' ';
  ...
  begin
    ...
    sAgencia := PadLeft(OnlyNumber(Titulo.Boleto.Cedente.Agencia) +
                        Titulo.Boleto.Cedente.AgenciaDigito,5,'0');
    ...
    sCCorrente := OnlyNumber(Titulo.Boleto.Cedente.Conta);
    sCCorrente := Copy(SCCorrente, Length(sCCorrente) - 8, 9) +
                  OnlyNumber(Titulo.Boleto.Cedente.ContaDigito);
    ...
    case Boleto.Cedente.TipoCarteira of
    ...
    case Boleto.Cedente.TipoDocumento of
    ...
    ISequencia := (Boleto.ListadeBoletos.IndexOf(Titulo) * 4) + 1;
    {SEGMENTO P}
    Result := '033'                                                   + // 001 - 003 / Código do Banco na compensação
              '0001'                                                  + // 004 - 007 / Numero do lote remessa
...
procedure TBancoSantander.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   aCarteira := StrToIntDef(Titulo.Carteira, 0 );
   ...
   if aCarteira = 5 then
      aAgencia := PadLeft(OnlyNumber(Titulo.Boleto.Cedente.Agencia) +
                       Titulo.Boleto.Cedente.AgenciaDigito,5,'0')
   else
      aAgencia:= '00000';

   vTotalTitulos:= vTotalTitulos+ Titulo.ValorDocumento;
   with Titulo do
   begin
      DigitoNossoNumero := CalcularDigitoVerificador(Titulo);
...
function TBancoSantander.GerarRegistroTrailler240(
  ARemessa: TStringList): String;
begin
// by Jéter Rabelo Ferreira - 06/2014
   {REGISTRO TRAILER DO LOTE}
   Result:= IntToStrZero(Banco.Numero, 3)                          + // 001 - 003 / Código do Banco na compensação
            '0001'                                                     + // 004 - 007 / Numero do lote remessa
   ...
   Result:= Result + #13#10 +
            IntToStrZero(Banco.Numero, 3)                          + // 001 - 003 / Código do Banco na compensação
...
procedure TBancoSantander.GerarRegistroTrailler400( ARemessa:TStringList );
var
  vQtdeLinha : Integer;
  wLinha: String;
begin
...
procedure TBancoSantander.LerRetorno240(ARetorno: TStringList);
var
  Titulo: TTitulo;
...
Procedure TBancoSantander.LerRetorno400 ( ARetorno: TStringList );
var
  Titulo : TTitulo;
...
function TBancoSantander.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoSantander.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoSantander.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoSantander.TipoOCorrenciaToCod (
...
function TBancoSantander.CodMotivoRejeicaoToDescricao( const TipoOcorrencia:TTipoOcorrencia; CodMotivo: Integer) : String;
begin
