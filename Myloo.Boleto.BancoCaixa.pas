...
type
  ...
  TCaixaEconomica = class(TBancoClass)
   protected
   ...
   private
    ...
    Function RetornaModalidade(const Titulo :TTitulo): String;
    Function FormataNossoNumero(const Titulo :TTitulo): String;
    Function RetornaCodCarteira(Carteira: string; const Titulo : TTitulo): integer;
   public
    Constructor create(AOwner: TBanco);
    Function CalcularDigitoVerificador(const Titulo: TTitulo ): String; override;
    Function CalcularDVCedente(const Titulo: TTitulo ): String;
    ...
    Function TipoOcorrenciaToCodRemessa(const TipoOcorrencia: TTipoOcorrencia): string; override;
   end;

implementation

uses
  StrUtils, Variants, DateUtils, Myloo.Boleto.Utils;

constructor TCaixaEconomica.create(AOwner: TBanco);
begin
...
function TCaixaEconomica.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
var
  Num, ACarteira, ANossoNumero, Res :String;
begin
   Result := '0';
   if (Titulo.Carteira = 'RG') then
      ACarteira := '1'
   else if (Titulo.Carteira = 'SR')then
      ACarteira := '2'
   else
      raise Exception.Create(Str('Carteira Inválida.'+sLineBreak+'Utilize "RG" ou "SR"') ) ;

   ANossoNumero := OnlyNumber(Titulo.NossoNumero);

   if Titulo.CarteiraEnvio = tceCedente then //O Cedente é quem envia o boleto
...
function TCaixaEconomica.CalcularDVCedente(const Titulo: TTitulo): String;
var
  Num, Res: string;
begin 
    Num := RightStr(Titulo.Boleto.Cedente.CodigoCedente,6);
    Modulo.CalculoPadrao;
    Modulo.MultiplicadorFinal   := 2;
    Modulo.MultiplicadorInicial := 9;
    Modulo.Documento := Num;
...
function TCaixaEconomica.GetLocalPagamento: String;
begin
  Result := Str(CInstrucaoPagamentoLoterica);
end;

function TCaixaEconomica.RetornaModalidade(const Titulo :TTitulo): String;
var
  AModalidade : String;
begin
   with Titulo do
   begin
    AModalidade := OnlyNumber(Boleto.Cedente.Modalidade);

      if (Titulo.Carteira = 'RG') then        {carterira registrada}
        begin
        if Titulo.CarteiraEnvio = tceCedente then
          AModalidade := '14'
        else
          AModalidade := '11'
      end
      else if (Titulo.Carteira = 'SR')then     {carteira 2 sem registro}
      begin
        if Titulo.CarteiraEnvio = tceCedente then
          AModalidade := '24'
        else
...
function TCaixaEconomica.FormataNossoNumero(const Titulo :TTitulo): String;
var
...
begin
   ...
   begin
      ...
      AModalidade:= RetornaModalidade(Titulo);

      if (Titulo.Carteira = 'RG') or (Titulo.Carteira = 'SR') then
      begin
        if Titulo.CarteiraEnvio = tceCedente then
...
function TCaixaEconomica.MontarCodigoBarras(const Titulo : TTitulo): String;
var
...
begin
    ...
    CampoLivre := PadLeft(Titulo.Boleto.Cedente.CodigoCedente,6,'0') +
                  CalcularDVCedente(Titulo) + Copy(ANossoNumero,3,3)  +
...
function TCaixaEconomica.RetornaCodCarteira(Carteira: string; const Titulo: TTitulo): integer;

begin
    if (Carteira = 'RG') and (Titulo.CarteiraEnvio = tceCedente) then
    Result := 14
    else if (Carteira = 'RG') and (Titulo.CarteiraEnvio = tceBanco) then
    Result := 11
    else if (Carteira = 'SR') and (Titulo.CarteiraEnvio = tceCedente) then
    Result := 24
    else if (Carteira = 'SR') and (Titulo.CarteiraEnvio = tceBanco) then
    Result := 21
    else
      raise Exception.Create(Str('Carteira Inválida.'+sLineBreak+'Utilize "RG" ou "SR"'));
end;

function TCaixaEconomica.TipoOCorrenciaToCod(const TipoOcorrencia: TTipoOcorrencia): String;
begin

  if (Banco.Boleto.LayoutRemessa = c240) then
  begin
...
function TCaixaEconomica.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
  Result := RightStr(Titulo.Boleto.Cedente.Agencia,4)+ '/' +
            Titulo.Boleto.Cedente.CodigoCedente+ '-' +
                CalcularDVCedente(Titulo);
end;

function TCaixaEconomica.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
var ANossoNumero : string;
begin
    ANossoNumero := FormataNossoNumero(Titulo);

    Result := ANossoNumero + '-' + CalcularDigitoVerificador(Titulo);
end;

function TCaixaEconomica.GerarRegistroHeader240(NumeroRemessa : Integer): String;
var
...
begin
   ...
   begin
      ...
      if Banco.Boleto.Homologacao then
      ...
      Result:= IntToStrZero(Banco.Numero, 3)                + //1 a 3 - Código do banco
      ...
      Result:= Result + #13#10 +
               IntToStrZero(Banco.Numero, 3)            +//1 a 3 - Código do banco
...
procedure TCaixaEconomica.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa: TStringList);
var
  wLinha: String;
begin
...
function TCaixaEconomica.GerarRegistroTransacao240(Titulo : TTitulo): String;
var
...
begin
   ...
   begin
      ...
      if ( Trim(Titulo.NossoNumero) <> '' ) then
        ANossoNumero := FormataNossoNumero(Titulo)
      else
        ANossoNumero := '';  
      ...
      Result:= IntToStrZero(Banco.Numero, 3)                          + //1 a 3 - Código do banco
               ...
               PadLeft(OnlyNumber(Boleto.Cedente.Agencia), 5, '0')       + //18 a 22 - Agência mantenedora da conta
               PadRight(Boleto.Cedente.AgenciaDigito, 1 , '0')            + //23 -Dígito verificador da agência
               PadRight(Boleto.Cedente.CodigoCedente, 6, '0')             + //24 a 29 - Código do Convênio no Banco (Codigo do cedente)
...
procedure TCaixaEconomica.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
  ATipoOcorrencia, ATipoBoleto, ADataMoraJuros, AModalidade  :String;
  ...
  begin
     ...
     begin
        ...
        Result := sLineBreak +
                  ...
                  PadLeft(OnlyNumber(Boleto.Cedente.CNPJCPF),14,'0')         + //   4 até 17 - Numero de Inscrição Empresa
                  RightStr(OnlyNumber(Boleto.Cedente.Agencia), 4 )           + //  18 até 21 - Código da Agência
                  PadLeft(OnlyNumber(Boleto.Cedente.CodigoCedente), 6, '0')  + //  22 até 27 - Código do Cedente
...
begin
   ...
   begin
      ...
      TamConvenioMaior6:= Length(trim(Boleto.Cedente.Convenio)) > 6;
      ...
      case Boleto.Cedente.TipoInscricao of
      ...
      begin
         ...
         wLinha:= wLinha                                                           + //  1 até 1   -  ID Registro
                  ATipoCendente                                                    + //  2 até 3   -  Tipo de inscrição da empresa 01-CPF / 02-CNPJ
                  PadLeft(OnlyNumber(Boleto.Cedente.CNPJCPF),14,'0')           + //  4  até 17 - Inscrição da empresa
                  RightStr(OnlyNumber(Boleto.Cedente.Agencia), 4 )             + // 18 até 21  - Código da Agência
                  PadLeft(OnlyNumber(Boleto.Cedente.CodigoCedente), 6, '0')    + // 22 até 27  - Código do Cedente
                  ATipoBoleto                                                      + // 28 a 29    - Quem emite e quem distribui
                  '00'                                                             + // 30 a 31    - Comissão de permanência - informar 00
                  PadLeft(OnlyNumber(Titulo.SeuNumero), 25)                    + // 32 até 56  - Seu numero
...
function TCaixaEconomica.GerarRegistroTrailler240( ARemessa : TStringList ): String;
var
  wQTDTitulos: Integer;
begin
   ...
   Result:= IntToStrZero(Banco.Numero, 3)                          + //Código do banco
   ...
   Result:= Result + #13#10 +
            IntToStrZero(Banco.Numero, 3)                          + //Código do banco
...
procedure TCaixaEconomica.GerarRegistroTrailler400(ARemessa: TStringList);
var
  wLinha: String;
begin
...
procedure TCaixaEconomica.LerRetorno240(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
...
begin
   ...
   if TempData <> '00/00/00' then
     Banco.Boleto.DataArquivo   := StringToDateTimeDef(TempData,0, 'DD/MM/YY');

   TempData := Copy(ARetorno[1],200,2) + '/' + Copy(ARetorno[1],202,2) + '/' +
               Copy(ARetorno[1],206,2);

   if TempData <> '00/00/00' then
     Banco.Boleto.DataCreditoLanc := StringToDateTimeDef(TempData, 0, 'DD/MM/YY');
...
function TCaixaEconomica.CodOcorrenciaToTipo(
  const CodOcorrencia: Integer): TTipoOcorrencia;
begin
...
function TCaixaEconomica.CodOcorrenciaToTipoRemessa(
  const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TCaixaEconomica.TipoOcorrenciaToCodRemessa(const TipoOcorrencia: TTipoOcorrencia): string;
begin
  case TipoOcorrencia of
...
function TCaixaEconomica.CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia; CodMotivo: Integer): string;
begin  
  if (Banco.Boleto.LayoutRemessa = c240) then
  begin
  case TipoOcorrencia of
...
function TCaixaEconomica.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TCaixaEconomica.CodigoLiquidacao_Descricao(CodLiquidacao: Integer): String;
begin
...
procedure TCaixaEconomica.LerRetorno400(ARetorno: TStringList);
var
  Titulo : TTitulo;
  ContLinha : Integer;
...
begin
   ...
   Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0],390,5),0);
