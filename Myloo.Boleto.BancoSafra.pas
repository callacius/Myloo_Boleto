...
type
  ...
  public
    ...
    procedure GerarRegistroTransacao400(Titulo: TTitulo;
      aRemessa: TStringList); override;
    ...
    Function TipoOCorrenciaToCod(const TipoOcorrencia: TTipoOcorrencia): string;
      override;
    Function CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia;
...
constructor TBancoSafra.Create(AOwner: TBanco);
begin
...
function TBancoSafra.CalcularDigitoVerificador(
...
function TBancoSafra.CodMotivoRejeicaoToDescricao(
...
function TBancoSafra.CodOcorrenciaToTipo(const CodOcorrencia: integer):
TTipoOcorrencia;
begin
...
function TBancoSafra.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
procedure TBancoSafra.GerarRegistroHeader400(NumeroRemessa: integer;
...
procedure TBancoSafra.GerarRegistroTrailler400(ARemessa: TStringList);
var
  wLinha: String;
begin
...
procedure TBancoSafra.GerarRegistroTransacao400(Titulo: TTitulo;
  aRemessa: TStringList);
var
...
begin
  ...
  begin
    ...
    aAgencia := PadLeft(Boleto.Cedente.Agencia, 4, '0') +
                PadLeft(Boleto.Cedente.AgenciaDigito, 1, '0');

    aConta := PadLeft(Boleto.Cedente.Conta, 8, '0') +
    ...
    wLinha := '1'                                                                            + // Identificação do Registro de Transação
              ...
              PadLeft(OnlyNumber(Boleto.Cedente.CNPJCPF), 14, '0')                       + // Número da Inscrição da Empresa
              ...
              IfThen(NossoNumero = '000000000', '000000000',
                                 ...
                                 CalcularDigitoVerificador(Titulo)) +                      // Número do título no banco
              Space(30)                                                                      + // "Brancos"
              ...
              IntToStr(Boleto.Banco.Numero)                                              + // Código do Banco encarregado da cobraça
...
procedure TBancoSafra.LerRetorno400(ARetorno: TStringList);
var
...
begin
  ...
  rAgencia       := trim(Copy(ARetorno[1], 18, Banco.TamanhoAgencia));
  rDigitoAgencia := trim(Copy(ARetorno[1], 22, 1));
  rConta         := trim(Copy(ARetorno[1], 23, Banco.TamanhoConta));
  ...
  Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0], 392, 3), 0);
...
function TBancoSafra.MontarCampoCodigoCedente(
  const Titulo: TTitulo): string;
begin
  with Titulo.Boleto.Cedente do
  begin
    Result := PadLeft(Agencia, 4, '0') + PadLeft(AgenciaDigito, 1, '0') + '/' + PadLeft(Boleto.Cedente.Conta, 8, '0') + PadLeft(Boleto.Cedente.ContaDigito, 1, '0');
  end;
end;

function TBancoSafra.MontarCampoNossoNumero(const Titulo: TTitulo): string;
begin
  with Titulo do
  begin
    Result := PadLeft(RightStr(NossoNumero,8),8,'0') + '-' + CalcularDigitoVerificador(Titulo);
  end;
end;

function TBancoSafra.MontarCodigoBarras(const Titulo: TTitulo): string;
var
...
begin
  ...
  begin
    ...
    CodigoBarras := IntToStr(Banco.Numero) + '9' + FatorVencimento +
                    ...
                    PadLeft(RightStr(Titulo.NossoNumero,8),8,'0') + CalcularDigitoVerificador(Titulo) + '2';
...
function TBancoSafra.TipoOCorrenciaToCod(
...
function TBancoSafra.TipoOcorrenciaToDescricao(
  const TipoOcorrencia: TTipoOcorrencia): string;
var
