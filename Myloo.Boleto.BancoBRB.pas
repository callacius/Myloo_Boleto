...
type
  ...
  public
    ...
    Function MontarChaveASBACE(const Titulo: TTitulo): string;
    Function CalculaDigitosChaveASBACE(ChaveASBACESemDigito: string): string;
    procedure LerRetorno400(ARetorno: TStringList); override;
    Function CodOcorrenciaToTipo(const CodOcorrencia:Integer): TTipoOcorrencia; overload; override;
  end;
...
constructor TBancoBRB.create(AOwner: TBanco);
begin
...
function TBancoBRB.CalculaDigitosChaveASBACE(ChaveASBACESemDigito: string): string;
...
function TBancoBRB.MontarCodigoBarras(const Titulo: TTitulo): string;
var
...
begin
   ...
   begin
      ...
      ChaveASBACE := MontarChaveASBACE(Titulo);
...
function TBancoBRB.MontarCampoNossoNumero(const Titulo: TTitulo): string;
begin
   Result := Copy(MontarChaveASBACE(Titulo), 14, 12);
end;

function TBancoBRB.MontarCampoCodigoCedente(const Titulo: TTitulo): string;
begin
   Result := '000-' +
             PadLeft(Titulo.Boleto.Cedente.Agencia, 3, '0') + '.' +
             FormatFloat('000,000', StrToFloat(Titulo.Boleto.Cedente.Conta)) + '-' +
             PadLeft(Titulo.Boleto.Cedente.ContaDigito, 1, '0');
end;

procedure TBancoBRB.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa: TStringList);
var
  wLinha: String;
begin
  ...
  begin
     wLinha := 'DCB'                                             + // Literal DCB
               ...
               IntToStrZero(Boleto.ListadeBoletos.Count +1,6); // Qtde de registros Header + Detalhe
     aRemessa.Text := aRemessa.Text + UpperCase(wLinha);
  end;
end;

procedure TBancoBRB.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
  TipoPessoa: Char;
...
begin
  ...
  begin
     ...
     { Nosso Número }
     if StrToIntDef(Boleto.Cedente.Modalidade, 1) in [1..2] then
        lNossoNumero := PadLeft(MontarCampoNossoNumero(Titulo), 12, '0')
     else
...
procedure TBancoBRB.LerRetorno400(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
  Linha: String ;
...
function TBancoBRB.CodOcorrenciaToTipo(
...
function TBancoBRB.MontarChaveASBACE(const Titulo: TTitulo): string;
var
  ChaveASBACESemDigito: string;
begin
  if trim(Titulo.Boleto.Cedente.Modalidade) = '' then
     raise Exception.Create(Str('Campo Modalidade não informado, impossivel continuar.'));

  ChaveASBACESemDigito := '000';
  ChaveASBACESemDigito := ChaveASBACESemDigito + PadLeft(OnlyNumber(Titulo.Boleto.Cedente.Agencia), 3, '0');
  ChaveASBACESemDigito := ChaveASBACESemDigito + PadLeft(OnlyNumber(Titulo.Boleto.Cedente.Conta), 6, '0');
  ChaveASBACESemDigito := ChaveASBACESemDigito + PadLeft(Titulo.Boleto.Cedente.ContaDigito, 1, '0');
  ChaveASBACESemDigito := ChaveASBACESemDigito + PadRight(trim(Titulo.Boleto.Cedente.Modalidade), 1); //Categoria da Cobrança
  ChaveASBACESemDigito := ChaveASBACESemDigito + PadLeft(IntToStr(StrToInt(Titulo.NossoNumero)), 6, '0');
