unit Myloo.Boleto.BancoBancoob;
...
constructor TBancoob.create(AOwner: TBanco);
begin
...
function TBancoob.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
var
  Num, Res :String;
...
begin
   ...
   Num :=  PadLeft(Titulo.Boleto.Cedente.Agencia, 4, '0') +
           PadLeft(Titulo.Boleto.Cedente.CodigoCedente, 10, '0') +
           PadLeft(trim(Titulo.NossoNumero), 7, '0');
...
function TBancoob.MontarCodigoBarras(const Titulo : TTitulo): String;
var
...
begin
    ...
    ANossoNumero := Titulo.NossoNumero+CalcularDigitoVerificador(Titulo);

    if (Titulo.Carteira = '1') or (Titulo.Carteira = '3')then
       ACarteira := Titulo.Carteira
    else
       raise Exception.Create(Str('Carteira Inválida.'+sLineBreak+'Utilize "1" ou "3".') );

    {Montando Campo Livre}
    CampoLivre    := PadLeft(trim(Titulo.Boleto.Cedente.Modalidade), 2, '0') +
                     PadLeft(trim(Titulo.Boleto.Cedente.CodigoCedente), 7, '0') +
                     PadLeft(Copy(ANossoNumero,1,8), 8, '0') +  //7 Sequenciais + 1 do digito
                     IntToStrZero(Max(1,Titulo.Parcela),3);
...
function TBancoob.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;

var
  CodigoCedente: String;
begin
  CodigoCedente := Titulo.Boleto.Cedente.CodigoCedente;
  Result := Titulo.Boleto.Cedente.Agencia + '/'+
            copy(CodigoCedente,1,length(CodigoCedente)-1)+ '-'+
            copy(CodigoCedente,length(CodigoCedente),1);
end;

function TBancoob.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
begin
  Result := Titulo.NossoNumero + '-' + CalcularDigitoVerificador(Titulo);
end;

procedure TBancoob.GerarRegistroHeader400(NumeroRemessa : Integer; aRemessa:TStringList);
var
  wLinha: String;
begin
...
procedure TBancoob.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
begin
    ...
    if (Length(Titulo.Carteira) < 1 )then
       raise Exception.Create(Str('Carteira Inválida.'+sLineBreak) ) ;
   ...
   begin
      ...
      if Boleto.Cedente.TipoInscricao  = pFisica then
         TipoCedente := '01'
      else
         TipoCedente := '02';
      ...
      if (Titulo.CarteiraEnvio = tceBanco) then
        strCarteiraEnvio := '1'
      else
        strCarteiraEnvio := '2';
...
procedure TBancoob.GerarRegistroTrailler400( ARemessa: TStringList );
var
  wLinha: String;
begin
...
procedure TBancoob.LerRetorno240(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
...
procedure TBancoob.LerRetorno400(ARetorno: TStringList);
var
  ContLinha: Integer;
  Titulo   : TTitulo;
  Linha, rCedente, rCNPJCPF : String;
begin
   ...
   Banco.Boleto.DataCreditoLanc := StringToDateTimeDef(Copy(ARetorno[1],111,2)+'/'+
...
function TBancoob.GerarRegistroHeader240(
...
function TBancoob.GerarRegistroTransacao240(
...
var AEspecieTitulo, ATipoInscricao, ATipoOcorrencia, ATipoBoleto, ADataMoraJuros,
    ...
    K: Integer;
begin
  if ( Titulo.NossoNumero <> IntToStrZero(0, length(Titulo.NossoNumero)) ) then
    NossoNum  := RemoveString('-', MontarCampoNossoNumero(Titulo))
  else
    ...
    begin
     ...
     if (Titulo.CarteiraEnvio = tceBanco) then
      ...
      Result:= IntToStrZero(Banco.Numero, 3)                             + //1 a 3 - Código do banco
               ...
               '0'                                                           + // 18
               PadLeft(OnlyNumber(Boleto.Cedente.Agencia),4,'0')         + //19 a 22 - Agência mantenedora da conta
               PadLeft(Boleto.Cedente.AgenciaDigito, 1, '0')             + //23 Digito agencia
               PadLeft(OnlyNumber(Boleto.Cedente.Conta), 12, '0')        + //24 a 35 - Número da Conta Corrente
               PadLeft(Boleto.Cedente.ContaDigito , 1, '0')              + //36 - Dígito da Conta Corrente
               ' ';
                                                                           //37 - DV Agência/COnta Brancos
               if (Boleto.Cedente.ResponEmissao = tbCliEmite) then
               begin
                  wModalidade:= ifthen(trim(Boleto.Cedente.Modalidade) = '',
                                       '02', Boleto.Cedente.Modalidade);
               ...
               else if ((Boleto.Cedente.ResponEmissao = tbBancoEmite) and (PadLeft(NossoNum, 10, '0') <> '0000000000')) then
               begin
                  Result := Format('%s000000000001014     ',[Result,PadLeft(wModalidade, 02, '0')]);
               end
               else if ((Boleto.Cedente.ResponEmissao = tbBancoEmite) and (PadLeft(NossoNum, 10, '0') = '0000000000')) then
               begin
                  Result := Format('%s000000000001014     ',[Result,PadLeft(wModalidade, 02, '0')]);
...
function TBancoob.GerarRegistroTrailler240(
...
function TBancoob.CodMotivoRejeicaoToDescricao(
...
function TBancoob.CodOcorrenciaToTipo(
...
function TBancoob.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoob.TipoOCorrenciaToCod(
...
function TBancoob.TipoOcorrenciaToDescricao(
  const TipoOcorrencia: TTipoOcorrencia): String;
var
