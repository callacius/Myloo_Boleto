...
constructor TBancoItau.create(AOwner: TBanco);
begin
...
function TBancoItau.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
var
  Docto: String;
begin
   Result := '0';
   ...
   begin
      ...
      '147','105','112','212','166','113','126','131','145','150','168']) then
            ...
            Docto := Boleto.Cedente.Agencia + Boleto.Cedente.Conta +
                     Carteira + PadLeft(Titulo.NossoNumero,TamanhoMaximoNossoNum,'0')
   end;
...
function TBancoItau.MontarCodigoBarras(const Titulo : TTitulo): String;
var
...
begin
    ...
    begin
       ...
       ANossoNumero := Titulo.Carteira +
                       PadLeft(Titulo.NossoNumero,8,'0') +
...
function TBancoItau.MontarCampoNossoNumero ( const Titulo: TTitulo
   ) : String;
var
  NossoNr: String;
begin
  ...
  Result := NossoNr + CalcularDigitoVerificador(Titulo);
end;

function TBancoItau.MontarCampoCodigoCedente (
...
function TBancoItau.GerarRegistroHeader240(NumeroRemessa : Integer): String;
var
...
function TBancoItau.GerarRegistroTransacao240(Titulo : TTitulo): String;
var
...
begin
   ...
   begin
      ...
      Result:= IntToStrZero(Banco.Numero, 3)                          + //1 a 3 - Código do banco
               ...
               PadLeft(OnlyNumber(Boleto.Cedente.Conta),5,'0')        + //31 a 35 - Número da Conta Corrente
               ' '                                                        + // 36
               Boleto.Cedente.ContaDigito                             + //37 - Dígito verificador da agência / conta
               PadLeft(Carteira, 3, ' ')                                  + // 38 a 40 - Carteira
               PadLeft(NossoNumero, 8, '0')                               + // 41 a 48 - Nosso número - identificação do título no banco
               CalcularDigitoVerificador(Titulo)                      + // 49 - Dígito verificador da agência / conta preencher somente em cobrança sem registro
      ...
      {Segmento R}
          ...
          Result:= Result + #13#10 +
                   IntToStrZero(Banco.Numero,3)                                    + // 001 a 003 - Codigo do Banco
...
function TBancoItau.GerarRegistroTrailler240( ARemessa : TStringList ): String;
var
  wRegsLote: Integer;
begin
...
procedure TBancoItau.GerarRegistroHeader400(
...
begin
   ...
   begin
      ...
      wLinha:=    '0'                                  + // 1 a 1     - IDENTIFICAÇÃO DO REGISTRO HEADER
                  ...
                  IntToStrZero(Banco.Numero, 3)    + // 77 a 79   - Nº DO BANCO NA CÂMARA DE COMPENSAÇÃO
...
procedure TBancoItau.GerarRegistroTransacao400( Titulo: TTitulo; aRemessa: TStringList);
var
    ...
    // Por padrão, essa rotina não será chamada
     ...
     begin
        ...
        Result := Result +
                  ...
                  IntToStrZero(ListadeBoletos.IndexOf(Titulo)+
                  ListadeBoletos.IndexOf(Titulo)+ 4, 6);                 // Nº SEQÜENCIAL DO REGISTRO NO ARQUIVO
    end;
  end;
       ...
       if (wCarteira = '102') or (wCarteira = '103') or
        ...
        begin
          wLinha:= '1'                                                                            + // 1 a 1 - IDENTIFICAÇÃO DO REGISTRO TRANSAÇÃO
                   ...
                   FormatDateTime('ddmmyy', Vencimento)                                           + // DATA DE VENCIMENTO DO TÍTULO
                   IntToStrZero( round( ValorDocumento * 100), 13)                                + // VALOR NOMINAL DO TÍTULO
                   IntToStrZero(Banco.Numero, 3)                                              + // Nº DO BANCO NA CÂMARA DE COMPENSAÇÃO
                   '00000'                                                                        + // AGÊNCIA ONDE O TÍTULO SERÁ COBRADO
                   PadLeft(ATipoEspecieDoc, 2, '0')                                               + // ESPÉCIE DO TÍTULO
                   ATipoAceite                                                                    + // IDENTIFICAÇÃO DE TITILO ACEITO OU NÃO ACEITO
                   FormatDateTime('ddmmyy', DataDocumento)                                        + // DATA DA EMISSÃO DO TÍTULO
                   PadLeft(trim(Str(Instrucao1)), 2, '0')                                     + // 1ª INSTRUÇÃO
                   PadLeft(trim(Str(Instrucao2)), 2, '0')                                     + // 2ª INSTRUÇÃO
...
procedure TBancoItau.GerarRegistroTrailler400(
...
procedure TBancoItau.LerRetorno240(ARetorno: TStringList);
var
  Titulo: TTitulo;
      ...
      begin
         if copy(Linha, 14, 1) = 'T' then
          begin
            ...
            NossoNumero := copy(Linha, 41, Banco.TamanhoMaximoNossoNum);
...
procedure TBancoItau.LerRetorno400(ARetorno: TStringList);
var
...
begin
  ...
  Banco.Boleto.DataCreditoLanc := StringToDateTimeDef(Copy(ARetorno[0],114,2)+'/'+
...
function TBancoItau.TipoOcorrenciaToDescricao(
...
function TBancoItau.CodOcorrenciaToTipo(
...
function TBancoItau.TipoOCorrenciaToCod(
  const TipoOcorrencia: TTipoOcorrencia): String;
begin
  Result := '';
...
function TBancoItau.CodMotivoRejeicaoToDescricao(
...
function TBancoItau.CodOcorrenciaToTipoRemessa(const CodOcorrencia:Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
