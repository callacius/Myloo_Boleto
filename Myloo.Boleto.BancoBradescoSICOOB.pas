unit Myloo.Boleto.BancoBradescoSICOOB;
...
type
  ...
  { TBancoBradescoSICOOB }

  TBancoBradescoSICOOB = class(TBancoClass)
  private
    FBoleto: TBoleto;
...
constructor TBancoBradescoSICOOB.create(AOwner: TBanco);
begin
...
function TBancoBradescoSICOOB.CalcularFatorVencimento(
  const DataVencimento: TDateTime): String;
begin
  Result := IntToStr( Trunc(DataVencimento - EncodeDate(2000,07,03)) + 1000);
end;

function TBancoBradescoSICOOB.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
   Modulo.CalculoPadrao;
   Modulo.MultiplicadorInicial := 2;
   Modulo.MultiplicadorFinal   := 7;
   Modulo.Documento := '9' + FormataNossoNumero( Titulo );
   Modulo.Calcular;
...
function TBancoBradescoSICOOB.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
...
begin
   ...
   begin
      ...
      NossoNumero := MontarCampoNossoNumero(Titulo);
      ...
      CodigoBarras := IntToStr( Numero )+'9'+ PadLeft(OnlyNumber(FatorVencimento),4,'0') +
                      ...
                      PadLeft(OnlyNumber(Titulo.Carteira),2,'0') +
...
function TBancoBradescoSICOOB.MontarCampoNossoNumero (
   const Titulo: TTitulo ) : String;
begin
   Result:=  FormatDateTime('yy',Titulo.DataDocumento)+
             PadLeft( Titulo.Boleto.Cedente.Convenio, 3, '0')+
             Titulo.NossoNumero+
             CalcularDigitoVerificador(Titulo);
end;

function TBancoBradescoSICOOB.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
...
procedure TBancoBradescoSICOOB.GerarRegistroHeader400(NumeroRemessa : Integer; ARemessa:TStringList);
var
  wLinha: String;
begin
   ...
   begin
      wLinha:= '0'                                             + // ID do Registro
               ...
               IntToStrZero( Banco.NumeroCorrespondente, 3  ) + PadRight('BANCOOB', 15)   + // Código e Nome do Banco(756 - Sicoob)
...
procedure TBancoBradescoSICOOB.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
procedure TBancoBradescoSICOOB.GerarRegistroTrailler400( ARemessa:TStringList );
var
  wLinha: String;
begin
...
Procedure TBancoBradescoSICOOB.LerRetorno400 ( ARetorno: TStringList );
var
  Titulo : TTitulo;
...
begin
   if ( StrToIntDef(copy(ARetorno.Strings[0],77,3),-1) <> Banco.NumeroCorrespondente ) then
      raise Exception.Create(Str(Banco.Boleto.NomeArqRetorno +
                             'não é um arquivo de retorno do '+ Nome));

   rCedente := trim(Copy(ARetorno[0],47,30));

   rAgencia := trim(Copy(ARetorno[0], 101, Banco.TamanhoAgencia));
   rConta   := trim(Copy(ARetorno[0], 106, Banco.TamanhoConta));
...
function TBancoBradescoSICOOB.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoBradescoSICOOB.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoBradescoSICOOB.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoBradescoSICOOB.TipoOcorrenciaToCod ( const TipoOcorrencia: TTipoOcorrencia ) : String;
begin
...
function TBancoBradescoSICOOB.COdMotivoRejeicaoToDescricao( const TipoOcorrencia:TTipoOcorrencia ;CodMotivo: Integer) : String;
begin
   case TipoOcorrencia of
...
function TBancoBradescoSICOOB.CalcularTamMaximoNossoNumero(
  const Carteira: String; NossoNumero: String; Convenio: String): Integer;
begin
  Result := Banco.TamanhoMaximoNossoNum;
end;

function TBancoBradescoSICOOB.FormataNossoNumero(
  const Titulo: TTitulo): String;
begin
  Result:=  FormatDateTime('yy',Titulo.DataDocumento)+
             PadLeft( Titulo.Boleto.Cedente.Convenio, 3, '0')+
