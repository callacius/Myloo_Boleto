unit Myloo.Boleto.BancoBradesco;
...
type
  ...
  TBancoBradesco = class(TBancoClass)
  private
    FBoleto: TBoleto;
...
constructor TBancoBradesco.create(AOwner: TBanco);
begin
...
function TBancoBradesco.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
...
function TBancoBradesco.MontarCodigoBarras ( const Titulo: TTitulo) : String;
var
...
function TBancoBradesco.GerarRegistroHeader240(NumeroRemessa: Integer): String;
var
...
procedure TBancoBradesco.GerarRegistroHeader400(NumeroRemessa : Integer; ARemessa:TStringList);
var
  wLinha: String;
begin
   with Banco.Boleto.Cedente do
...
function TBancoBradesco.GerarRegistroTransacao240(Titulo: TTitulo): String;
var
  ATipoOcorrencia,
  ATipoBoleto,
...
begin
  Fsequencia     :=3 * Titulo.Boleto.ListadeBoletos.IndexOf(Titulo);
  TipoMovimento  :='1';

  case Titulo.CaracTitulo of
  ...
  begin
    ...
    FdigitoNossoNumero:=CalcularDigitoVerificador(Titulo);
    ...
    PadLeft(OnlyNumber(Titulo.Boleto.Cedente.Agencia), 5, '0')                            + //18 a 22 - Agência mantenedora da conta
    ...
    Padleft(Boleto.Cedente.ContaDigito, 1 , '0')     + //36 a 36 Dígito Verificador da Conta Alfa *G011
    ...
    PadLeft(Titulo.Carteira, 3, '0')                 + //38-40 Identificação do Produto 38 40 3 Num *G069
    ...
    {SEGMENTO Q}
    Result:= Result +sLineBreak+
    IntToStrZero(Banco.Numero, 3)                   + //Código do Banco na Compensação 1 3 3 - Num G001
    ...
    PadLeft(copy(OnlyNumber(Titulo.Sacado.CEP),0,5), 5, '0')                     + //CEP 129 133 5 - Num G034
    PadRight(copy(OnlyNumber(Titulo.Sacado.CEP),length(OnlyNumber(Titulo.Sacado.CEP))-2,3), 3, ' ')       + //Sufixo do CEP 134 136 3 - Num G035
...
procedure TBancoBradesco.GerarRegistroTransacao400(Titulo :TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   with Titulo do
   begin
      ANossoNumero := PadLeft(OnlyNumber(Titulo.NossoNumero),11, '0');

      if (Boleto.Cedente.ResponEmissao = tbBancoEmite) and (StrToInt64Def(ANossoNumero,0) = 0) then
        DigitoNossoNumero := '0'
      else
      begin
      ...
      aAgencia := IntToStrZero(StrToIntDef(OnlyNumber(Boleto.Cedente.Agencia),0),5);
      aConta   := IntToStrZero(StrToIntDef(OnlyNumber(Boleto.Cedente.Conta),0),7);
...
function TBancoBradesco.GerarRegistroTrailler240(ARemessa: TStringList): String;
var
  wQTDTitulos: Integer;
  wlinha:string;
begin
   ...
   wlinha:=IntToStrZero(Banco.Numero, 3)                          + //Código do Banco na Compensação 1 3 3 - Num G001
            '0001'                                                     + //Lote Lote de Serviço 4 7 4 - Num *G002
...
procedure TBancoBradesco.GerarRegistroTrailler400( ARemessa:TStringList );
var
  wLinha: String;
begin
...
Procedure TBancoBradesco.LerRetorno400 ( ARetorno: TStringList );
var
  Titulo : TTitulo;
...
//   rAgencia := trim(Copy(ARetorno[1], 25, Banco.TamanhoAgencia));
   rAgencia := trim(Copy(ARetorno[1], 26, Banco.TamanhoAgencia));
   rConta   := trim(Copy(ARetorno[1], 30, Banco.TamanhoConta));

   rDigitoConta := Copy(ARetorno[1],37,1);

   Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0],109,5),0);
...
//         raise Exception.Create(Str('Código da Empresa do arquivo inválido'));
...
function TBancoBradesco.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoBradesco.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoBradesco.TipoOcorrenciaToCod ( const TipoOcorrencia: TTipoOcorrencia ) : String;
begin
  Result := '';
...
function TBancoBradesco.COdMotivoRejeicaoToDescricao( const TipoOcorrencia:TTipoOcorrencia ;CodMotivo: Integer) : String;
begin
   case TipoOcorrencia of
...
procedure TBancoBradesco.LerRetorno240(ARetorno: TStringList);   // MONTANDO A LEITURA DO ARQUIVO DE RETORNO AQUI.... CLAUDIO
var
  Titulo: TTitulo;
...
function TBancoBradesco.CodOcorrenciaToTipoRemessa(const CodOcorrencia:Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
