...
const
  CBancoBrasil_Versao = '0.0.1';

type
  TBancoBrasil = class(TBancoClass)
   protected
   private
    FQtMsg: Integer;
    FBoleto: TBoleto;
   ...
   public
    ...
    Function MontarCampoCarteira(const Titulo: TTitulo): String; override;
...
uses
  DateUtils, StrUtils, Variants, Myloo.Boleto.Utils;

{ TBancoBrasil}

constructor TBancoBrasil.create(AOwner: TBanco);
begin
...
function TBancoBrasil.CalcularDigitoVerificador(const Titulo: TTitulo ): String;
begin
   Result := '0';
...
function TBancoBrasil.CalcularTamMaximoNossoNumero(
...
begin
   ...
   if (Banco.Boleto.Cedente.Convenio = '') then
      raise Exception.Create(Str('Banco do Brasil requer que o Convênio do Cedente '+
                                     'seja informado.'));

   if (Carteira = '') then
      raise Exception.Create(Str('Banco do Brasil requer que a carteira seja '+
                                     'informada antes do Nosso Número.'));

   wCarteira:= Trim(Carteira);
   wTamConvenio:= Length(Trim(Banco.Boleto.Cedente.Convenio));
...
function TBancoBrasil.FormataNossoNumero(const Titulo :TTitulo): String;
var
...
function TBancoBrasil.NossoNumeroSemFormatacaoLerRetorno(const Convenio,
...
function TBancoBrasil.MontarCodigoBarras(const Titulo : TTitulo): String;
var
  CodigoBarras, FatorVencimento, DigitoCodBarras :String;
  ANossoNumero, AConvenio: String;
  wTamNossNum: Integer;
begin
   AConvenio    := Titulo.Boleto.Cedente.Convenio;
   ANossoNumero := FormataNossoNumero(Titulo);
   wTamNossNum  := CalcularTamMaximoNossoNumero(Titulo.Carteira,
                                                Titulo.NossoNumero);
   ...
   begin
      ...
      if ((Titulo.Carteira = '18') or (Titulo.Carteira = '16')) and
...
function TBancoBrasil.MontarCampoCodigoCedente (
   const Titulo: TTitulo ) : String;
begin
   Result := Titulo.Boleto.Cedente.Agencia+'-'+
             Titulo.Boleto.Cedente.AgenciaDigito+'/'+
             IntToStr(StrToIntDef(Titulo.Boleto.Cedente.Conta,0)) +'-'+
             Titulo.Boleto.Cedente.ContaDigito;
end;

function TBancoBrasil.MontarCampoCarteira(const Titulo: TTitulo
  ): String;
begin
  Result := IfThen(Titulo.Boleto.Cedente.Modalidade = '',
                   Titulo.Carteira,
                   Titulo.Carteira + '/' + Titulo.Boleto.Cedente.Modalidade );
end;

function TBancoBrasil.MontarCampoNossoNumero (const Titulo: TTitulo ) : String;
var
  ANossoNumero :string;
  wTamConvenio, wTamNossoNum :Integer;
begin
   ANossoNumero := FormataNossoNumero(Titulo);
   wTamConvenio := Length(Banco.Boleto.Cedente.Convenio);
   wTamNossoNum := CalcularTamMaximoNossoNumero(Titulo.Carteira,
                                                OnlyNumber(Titulo.NossoNumero));
...
function TBancoBrasil.GerarRegistroHeader240(NumeroRemessa : Integer): String;
var
...
begin
   ...
   begin
      ...
      Result:= IntToStrZero(Banco.Numero, 3)               + // 1 a 3 - Código do banco
               ...
               Banco.Boleto.ListadeBoletos[0].Carteira + // 46 a 47 - Carteira
...
function TBancoBrasil.GerarRegistroTransacao240(Titulo : TTitulo): String;
var
...
begin
   ...
   begin
     ...
     if (((wCarteira = 11) or (wCarteira= 31) or (wCarteira = 51)) or
        ...
        ((Boleto.Cedente.ResponEmissao <> tbCliEmite) and
     ...
     aAgencia := PadLeft(Boleto.Cedente.Agencia, 5, '0');
     aConta   := PadLeft(Boleto.Cedente.Conta, 12, '0');
     ...
     Result:= IntToStrZero(Banco.Numero, 3)                                         + // 1 a 3 - Código do banco
              ...
              IntToStrZero((4 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 1 , 5) + // 9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
              ...
              PadRight(Boleto.Cedente.ContaDigito, 1, '0')                          + // 36 - Dígito verificador da conta
              ...
              InttoStr(Integer(Boleto.Cedente.TipoDocumento))                       + // 60 - Tipo de documento: Tradicional
     ...
     {SEGMENTO Q}
     Result:= Result + #13#10 +
              IntToStrZero(Banco.Numero, 3)                                        + // 1 - 3 Código do banco
              '0001'                                                                   + // 4 - 7 Número do lote
              '3'                                                                      + // 8 - 8 Tipo do registro: Registro detalhe
              IntToStrZero((4 * Boleto.ListadeBoletos.IndexOf(Titulo)) + 2 ,5) + // 9 - 13 Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
     ...
     Result:= Result + #13#10 +
              ...
              '0001'                                                                  + // 4 - 7 Número do lote
              '3'                                                                     + // 8 - 8 Tipo do registro: Registro detalhe
              IntToStrZero((4 * Boleto.ListadeBoletos.IndexOf(Titulo))+ 3 ,5) + // 9 - 13 Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
     ...
     begin
       Result := Result + #13#10 +
                IntToStrZero(Banco.Numero, 3)                                           + // 001 a 003 - Código do banco
                '0001'                                                                      + // 004 - 007 - Numero do lote remessa
                '3'                                                                         + // 008 - 008 - Tipo de registro
                IntToStrZero((4 * Boleto.ListadeBoletos.IndexOf(Titulo))+ 4 ,5)     + // 009 - 013 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
...
function TBancoBrasil.GerarRegistroTrailler240( ARemessa : TStringList ): String;
var
  wRegsLote: Integer;
begin
   ...
   Result:= IntToStrZero(Banco.Numero, 3)                              + // 1 - 3 Código do banco
            '0001'                                                         + // 4 - 7 Número do lote
...
procedure TBancoBrasil.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa:TStringList);
var
  ...
  begin
    ...
    if TamConvenioMaior6 then
      wLinha:= wLinha + Space(22)                                        + // Nr. Sequencial de Remessa + brancos
               PadLeft(trim(Banco.Boleto.Cedente.Convenio),7,'0')+ // Nr. Convenio
               space(258)                                                  // Brancos
    else
...
procedure TBancoBrasil.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
...
begin
   ...
   begin
     ...
     aAgencia         := PadLeft(Boleto.Cedente.Agencia, 4, '0');
     aConta           := RightStr(Boleto.Cedente.Conta, 8);
     aModalidade      := PadLeft(trim(Boleto.Cedente.Modalidade), 3, '0');
     ...
     case StrToInt(Titulo.Carteira) of
       11,17 :
          case Boleto.Cedente.CaracTitulo of
...
procedure TBancoBrasil.GerarRegistroTrailler400(
...
procedure TBancoBrasil.LerRetorno240(ARetorno: TStringList);
var
  Titulo: TTitulo;
      ...
      begin
         if copy(Linha, 14, 1) = 'T' then
          begin
            ...
            NossoNumero := NossoNumeroSemFormatacaoLerRetorno(Boleto.Cedente.Convenio, Carteira, Linha);
...
function TBancoBrasil.TipoOCorrenciaToCod (
...
function TBancoBrasil.TipoOcorrenciaToDescricao(const TipoOcorrencia: TTipoOcorrencia): String;
var
...
function TBancoBrasil.CodOcorrenciaToTipo(const CodOcorrencia:
...
function TBancoBrasil.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoBrasil.CodMotivoRejeicaoToDescricao(const TipoOcorrencia: TTipoOcorrencia; CodMotivo: Integer): String;
begin

    if (Banco.Boleto.LayoutRemessa = c400) then
    begin
  case TipoOcorrencia of
    ...
    toRetornoBaixaAutomatica, toRetornoBaixaSolicitada, toRetornoDebitoEmConta: // 09, 10 ou 20 (Baixa)
      case CodMotivo of
        ...
        15: 
          case Banco.Boleto.LayoutRemessa of
...
procedure TBancoBrasil.LerRetorno400(ARetorno: TStringList);
var
 TamConvenioMaior6: Boolean;
begin
 TamConvenioMaior6:= Length(trim(Banco.Boleto.Cedente.Convenio)) > 6;
 if TamConvenioMaior6 then
...
procedure TBancoBrasil.LerRetorno400Pos6(ARetorno: TStringList);
var
  Titulo : TTitulo;
...
begin
   ...
   Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno[0],101,7),0);
   ...
   Banco.TamanhoMaximoNossoNum := FpTamanhoMaximoNossoNum;
...
procedure TBancoBrasil.LerRetorno400Pos7(ARetorno: TStringList);
var
  Titulo : TTitulo;
