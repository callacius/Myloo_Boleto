unit Myloo.Boleto.BancoBanrisul;
...
type
  ...
  TBancoBanrisul=class(TBancoClass)
  private
    FBoleto: TBoleto;
...
uses
  System.dateutils, System.StrUtils, Myloo.Boleto.Utils, Myloo.Validador;
...
constructor TBancoBanrisul.create(AOwner: TBanco);
begin
  ...
  FpOrientacoesBanco.Add(Str('SAC       BANRISUL - 0800 646 1515'+sLineBreak+
...
function TBancoBanrisul.CalculaDigitosChaveASBACE(ChaveASBACESemDigito: string): string;
...
function TBancoBanrisul.MontarCodigoBarras(const Titulo: TTitulo): string;
var
...
begin
  ...
  begin
     ...
     CampoLivre:= Modalidade +'1'+
                  PadLeft(copy(trim(Boleto.Cedente.Agencia),1,4), 4, '0')+{ Código agência (cooperativa) }
                  PadLeft(OnlyNumber(Boleto.Cedente.CodigoCedente), 7, '0')+{ Código cedente = codigoCedente }
...
function TBancoBanrisul.MontarCampoNossoNumero(const Titulo: TTitulo): string;
begin
  Result:=PadLeft(Titulo.NossoNumero, 8, '0');
  result:=Result+'.'+CalculaDigitosChaveASBACE(Result);
end;

function TBancoBanrisul.MontarCampoCodigoCedente(const Titulo: TTitulo): string;
begin
  // Código da Agência
  Result := copy(Titulo.Boleto.Cedente.Agencia, 1, 4);
  // Prosseguir somente se a Agência tiver dígito
  if Titulo.Boleto.Cedente.AgenciaDigito <> '' then
    begin
      // Separador do dígito da Agência
      Result := Result + '-';
      // Dígito da Agência
      Result := Result + Titulo.Boleto.Cedente.AgenciaDigito;
    end;
  // Separador entre a Agência e o Código Cedente
  Result := Result + '/';
  // Código do cedente
  Result := Result + Copy(Titulo.Boleto.Cedente.CodigoCedente,1,6) + '.';
  Result := Result + Copy(Titulo.Boleto.Cedente.CodigoCedente,7,1) + '.';
  Result := Result + Copy(Titulo.Boleto.Cedente.CodigoCedente,8,2);
end;

procedure TBancoBanrisul.GerarRegistroHeader400(NumeroRemessa: Integer; aRemessa: TStringList);
var
  cd,wLinha: string;
begin
  aTotal:=0;
...
procedure TBancoBanrisul.GerarRegistroTransacao400(Titulo: TTitulo; aRemessa: TStringList);
var
  AMensagem,
...
procedure TBancoBanrisul.GerarRegistroTrailler400(ARemessa: TStringList);
var
  wLinha: String;
begin
...
function TBancoBanrisul.GerarRegistroHeader240(
...
begin
  ...
  begin
     Result := '041'+                                             //   1 -   3   Código do banco
               ...
               PadRight(UpperCase(Banco.Nome), 30) +          // 103 - 132   Nome do banco
...
function TBancoBanrisul.GerarRegistroTrailler240(
...
function TBancoBanrisul.GerarRegistroTransacao240(
...
begin
   with Titulo do begin
      ...
      Result := '04100013' +
                ...
                PadLeft(OnlyNumber(Banco.Boleto.Cedente.Agencia), 5, '0') +
                // Dígito verificador da agência 23 - 23
                Banco.Boleto.Cedente.AgenciaDigito +
                // Número da conta corrente 24 - 35
                PadLeft(OnlyNumber(Banco.Boleto.Cedente.Conta), 12, '0') +
                // Dígito verificador da conta 36 - 36
                Banco.Boleto.Cedente.ContaDigito +
                // Dígito verificador da ag/conta 37 - 37
                Space(1) +


                PadLeft(OnlyNumber(MontarCampoNossoNumero(Titulo)), 10, '0') +
                DupeString(' ', 10) +
                ACaracTitulo +
                '1020' +
...
procedure TBancoBanrisul.LerRetorno240(ARetorno: TStringList);
var Titulo: TTitulo;
    FSegT, FSegU: String;
...
begin
  ...
  Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno.Strings[0], 158, 6), 0);

  Banco.Boleto.DataArquivo   := StringToDateTimeDef(Copy(ARetorno.Strings[0], 144, 2) +'/'+
                                                            Copy(ARetorno.Strings[0], 146, 2) +'/'+
                                                            Copy(ARetorno.Strings[0], 148, 4),
                                                            0, 'dd/mm/yyyy');

  Banco.Boleto. DataCreditoLanc := StringToDateTimeDef(Copy(ARetorno.Strings[1], 200, 2) +'/'+
  ...
  try
    ...
    try
      ...
      begin
        raise Exception.Create(Str('No arquivo de retorno "'+ Banco.Boleto.NomeArqRetorno +
    ...
    raise Exception.Create(Str('Não Foi possível realizar a leitura do arquivo de retorno "'+
                       Banco.Boleto.NomeArqRetorno +'" do(a) '+ UpperCase(Nome)));
  end;
end;

procedure TBancoBanrisul.LerRetorno400(ARetorno: TStringList);
var Titulo: TTitulo;
    Linha: String;
...
begin
  ...
  Banco.Boleto.NumeroArquivo := StrToIntDef(Copy(ARetorno.Strings[0], 386, 9), 0);
  Banco.Boleto.DataArquivo   := StringToDateTimeDef(Copy(ARetorno.Strings[0], 95, 2) +'/'+
  ...
  begin
    ...
    (rConta <> OnlyNumber(Cedente.CodigoCedente))) then
      raise Exception.Create(Str('Agência\Conta do arquivo inválido'));
...
function TBancoBanrisul.CodMotivoRejeicaoToDescricao(
...
function TBancoBanrisul.CodOcorrenciaToTipo(
...
function TBancoBanrisul.CodOcorrenciaToTipoRemessa(const CodOcorrencia: Integer): TTipoOcorrencia;
begin
  case CodOcorrencia of
...
function TBancoBanrisul.TipoOcorrenciaToDescricao(
...
function TBancoBanrisul.TipoOCorrenciaToCod(
  const TipoOcorrencia: TTipoOcorrencia): String;
begin
  Result := '';
